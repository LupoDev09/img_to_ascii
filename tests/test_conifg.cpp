...
TEST_CASE("Konfigurationsdatei wird korrekt geladen", "[config]") {
  std::string output =
      run_cmd("./img_to_ascii --load-config  ../example_config.json");
...
TEST_CASE("Ungültiger Pfad zur Konfigurationsdatei wird korrekt behandelt",
          "[config]") {
  std::string output =
      run_cmd("./img_to_ascii --load-config invalid_config.json");
  REQUIRE(output.find("Config nicht gefunden") != std::string::npos);
}

TEST_CASE("Konfigurationsdatei mit ungültiger endung", "[config]") {
  std::string output = run_cmd(
      "./img_to_ascii --load-config ../invalid_config.mit_ausgedachter_endung");
  REQUIRE(output.find("Config nicht gefunden") != std::string::npos);
}

TEST_CASE("Kommandozeilenargumente überschreiben Konfigurationsdatei",
          "[config]") {
  std::string output = run_cmd("./img_to_ascii --load-config "
                               "../example_config.json --width 150 --fps 20");
  REQUIRE(output.find("Konfigurationsdatei geladen") != std::string::npos);
