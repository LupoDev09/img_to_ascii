 ...
 *
 * @throws std::runtime_error Falls das Bild nicht geladen werden kann oder zu
 * wenige Kanäle hat.
 */
string image_to_ascii(const string &filename, const int output_width,
  ...
  verbose("img loaded");
  constexpr int used_channels =
      desired_channels; // buffer wird mit dieser Menge an kanälen zurückgegeben
  if (channels_in_file < 3) {
    // Warnung, aber nicht fatal: wir haben durch forced channels trotzdem RGB
    cerr << "Warnung: Quelldatei hat nur " << channels_in_file
         << " Kanäle; konvertiere zu RGB.\n";
  }
  ...
  const int output_height =
      static_cast<int>(static_cast<float>(output_width) * aspect_ratio * 0.55f);
  // 0.55 für Konsolen-Zeichenhöhe korrigiert

  string ascii;
  // Reserviere Platz im Voraus für Performance
  ascii.reserve(
      static_cast<unsigned long>(output_width * output_height + output_height));
  verbose("reserved space for ascii string");

  // Schrittgrößen fürs Sampling (Skalierung)
  const float x_step =
      static_cast<float>(width) / static_cast<float>(output_width);
  const float y_step =
      static_cast<float>(height) / static_cast<float>(output_height);
  ...
  for (int y = 0; y < output_height; y++) {
    for (int x = 0; x < output_width; x++) {
      ...
      // Transparenz prüfen, falls die Quelldatei tatsächlich ein Alpha-Kanal
      // hat
      if (channels_in_file >= 4) {
        const unsigned char a = img[idx + 3];
        if (a < 128) {
          // Pixel halbtransparent oder unsichtbar → Leerzeichen
          ascii.push_back(' ');
          continue;
        }
      }
      ...
      const unsigned long char_index = static_cast<unsigned long>(
          gray * static_cast<double>(ascii_chars.size() - 1) / 255.0);
 ...
 * wenige Kanäle hat.
 */
string image_to_ascii_color(const string &filename, const int output_width,
                            const string &ascii_chars) {
  ...
  string ascii;
  ascii.reserve(static_cast<unsigned long>(output_width * output_height *
                                           30)); // Platz für ANSI-Codes
  verbose("reserved space for ascii string");
