...
private:
    ...
    std::mutex mutex;                   ///< Mutex protecting access to the queue
    std::condition_variable condition;  ///< Condition variable used to notify the worker thread when new queue items are available
